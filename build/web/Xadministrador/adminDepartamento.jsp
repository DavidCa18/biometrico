...
<html>
    ...
    <body class="skin-1">
                        ...
                        <div class="col-md-12">
                            <div class="ibox-title">
                                <div class="form-group">
                                    <button type="button" class="btn btn-w-m btn-success" data-toggle="modal" data-target="#mdDepartamento" id="guardar"><i class="fa fa-plus"></i> Agregar Departamento</button>
                                </div>
                                <div class="table-responsive" >
                                    <table id="tbDepartamento" class="table table-striped table-bordered table-hover" style="width: 100%">
                                        <thead>
                                            <tr>
                                                <th style="background: #3B93D1; color: white">Código</th>
                                                <th style="background: #3B93D1; color: white">Nombre</th>
                                                <th style="background: #3B93D1; color: white">Descripción</th>
        ...
        <div class="modal fade" id="mdDepartamento" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content animated flipInY">
                    <form role="form" method="POST" id="fmrDepartamento" name="fmrDepartamento">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                            <h2 class="modal-title font-bold">Gestión de Departamentos</h2>
                        </div>
                        <div class="modal-body">

                            <div class="form-group">

                                <input type="number" class="form-control" id="txtCodigoD" name="txtCodigoD" style="display: none">
                            </div>
                            <div class="form-group">
                                <label>Nombre</label>
                                <input type="text" placeholder="Ingresar Nombre Departamento" class="form-control" id="txtNombreD" name="txtNombreD" >
                            </div>
                            <div class="form-group">
                                <label>Descripción</label>
                                <textarea placeholder="Ingresar Descripción Departamento" class="form-control" id="txtDescripD" name="txtDescripD" rows="2"></textarea>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-white" data-dismiss="modal"><i class="fa fa-times"></i>  Cancelar</button>
                            <button type="submit" class="btn btn-warning" id="btnModificarDepartamento"><i class="fa fa-pencil"></i> Modificar</button>
                            <button type="submit" class="btn btn-success" id="btnGuardarDepartamento"><i class="fa fa-save"></i> Guardar</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <script>
            ...
            $('#btnGuardarDepartamento').click(function (e) {
                e.preventDefault();
                if (Validar()) {
                    GuardarDepartamento();
                }
            ...
            $('#btnModificarDepartamento').click(function (e) {
                e.preventDefault();
                if (Validar()) {
            ...
            $('#guardar').click(function (e) {
                Limpiar();
                $("#btnModificarDepartamento").css("display", "none");
                $("#btnGuardarDepartamento").css("display", "inline");
            });

            function BuscarDepartamento() {

                var table = $('#tbDepartamento').DataTable({
                    "ajax": "../ServletDepartamentoBuscar",
                    destroy: true,
                    empty: true,
                    "columnDefs": [{
                            "targets": -2,
                ...
                $('#tbDepartamento tbody').on('click', '#btnModificar', function () {
                    var data = table.row($(this).parents('tr')).data();

                    $("#btnGuardarDepartamento").css("display", "none");
                    $("#btnModificarDepartamento").css("display", "inline");
                    $('#mdDepartamento').modal('show');

                    $('#txtCodigoD').val(data[0]);
                    $('#txtNombreD').val(data[1]);
                    $('#txtDescripD').val(data[2]);

                });

                $('#tbDepartamento tbody').on('click', '#btnEliminar', function () {
                    var data = table.row($(this).parents('tr')).data();

                    swal({
                        title: "¿Desea Eliminar el Registro?",
                        text: "El Departamento " + data[1] + " será Eliminado",
                        type: "warning",
            ...
            function GuardarDepartamento() {
                ...
                var data = $('#fmrDepartamento').serialize();

                $.ajax({
                    type: "POST",
                    url: "../ServletDepartamentoGuardar",
                    data: data,
                    success: function (responseText) {
                        toastr.success(responseText, 'Gestión Departamento');
                        BuscarDepartamento();
                        Limpiar();
                        $('#mdDepartamento').modal('toggle');
                    }
                }).fail(function (xhr, ajaxOptions, thrownError) {
                    toastr.error('Error al Guardar Departamento', 'Gestión Departamento');
                });
            }
            ...
            function ModificarDepartamento() {
                ...
                $.ajax({
                    type: "POST",
                    url: "../ServletDepartamentoModificar",
                    data: data,
                    success: function (responseText) {
                        toastr.success(responseText, 'Gestión Departamento');
                        BuscarDepartamento();
                    ...
                    toastr.error('Error al Modificar Departamento', 'Gestión Departamento');
                });
            }

            function EliminarDepartamento(txtCodigoD) {

                $.ajax({
                    type: "POST",
                    url: "../ServletDepartamentoEliminar",
                    data: {txtCodigoD: txtCodigoD},
                    success: function (responseText) {
                        toastr.success(responseText, 'Gestión Departamento');
                        BuscarDepartamento();
                        Limpiar();
                    ...
                    toastr.error('Error al Eliminar Departamento', 'Gestión Departamento');
                });
            }

            function Validar() {

                if ($('#txtNombreD').val().length > 0) {
                    if ($('#txtDescripD').val().length > 0) {
                        return true;
                    } else {
                        swal("Advertencia", "Ingresar Descripción Departamento", "warning");
                    }
                } else {
                    swal("Advertencia", "Ingresar Nombre Departamento", "warning");
                }
